...
public class DistortedLabelContextual extends AbstractImperfectLabelSimilarity {
			...
			for (int j = 0; j < parser.getActivities().size(); j++) {
				...
				if (j > i) {
					...
					if (overS > getOptions().get("Overall Context Similarity Threshold").asDouble()
						...
						addResult(selectedColumn, a1.getName(), a2.getName(), overS, ls[i][j], dcfs[i][j], rs[i][j],
